...
shell.mv('dist/es2015--es2015/bilderrahmen.js', 'dist/bilderrahmen.es2015.es2015.js');
shell.rm('-rf', './dist/es2015--es2015');

//
// COMPRESS UMD BUNDLE
//
shell.exec('uglifyjs  --compress --mangle --reserved "Bilderrahmen" --comments --output dist/bilderrahmen.umd.es5.min.js  dist/bilderrahmen.umd.es5.js');
...
shell.exec('cssnano --no-zindex src/bilderrahmen.css dist/bilderrahmen.min.css');

//
// WRITE FINAL LIB package.json
//
const templatePackageJson = JSON.parse(fs.readFileSync(relativePath('./build-package-json-template.json')));
const sourcePackageJson = JSON.parse(fs.readFileSync(relativePath('./package.json')));
// set values
templatePackageJson.version = sourcePackageJson.version;
templatePackageJson.description = sourcePackageJson.description;
fs.writeFileSync(relativePath('./dist/package.json'), JSON.stringify(templatePackageJson, null, 2));
